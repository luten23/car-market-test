...
import { postProperty, getAllProperties } from "../../store/actions"
	...
	return (
		<div className="cotainer__addItem-inner">
			...
			<Formik initialValues={{ name: "", type: undefined }}
				...
				onSubmit={values => {
					dispatch(postProperty(values.name, values.type)) // добовление проперти
					history.push("/itemsandproperty")
					setTimeout(() => dispatch(getAllProperties()), 100) // обновление списка всех проперти - через 0,1 сек - костыльгрёбаный
				}}
			>
