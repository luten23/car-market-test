...
export function ItemListTableMaterial({ searchReq }) {
	...
	//запрос списка item'ов в state + поиск
	React.useEffect(() => {
		dispatch(getAllItems(searchReq.searchReq))
	}, [searchReq, dispatch, rows.length])
										...
										return (
											...
											>
												<TableCell component="th" id={labelId} scope="row">
													<Link style={{color: "#0258ff", paddingLeft: "50px"}} to={`/items/${row.id}`}>{row.name}</Link>
												</TableCell>
												...
												<TableCell align="center"><ItemListButtons id={row.id} /></TableCell>
											</TableRow>
										);
									})}
								{emptyRows > 0 && (
